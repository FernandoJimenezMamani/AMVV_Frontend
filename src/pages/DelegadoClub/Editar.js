...
const EditarDelegado = ({ isOpen, onClose, delegadoId, onDelegadoUpdated }) => {
  ...
  const handleSubmitProfile = async (e) => {
    ...
    try {
        const response = await axios.put(`${API_BASE_URL}/persona/update_persona_with_roles/${delegadoId}`, formDataToSend, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
      console.log('Respuesta del backend:', response.data);
      toast.success('Delegado actualizado exitosamente');
      onClose();
      onDelegadoUpdated();
    } catch (error) {
