...
const RegistroJugadorClub = ({ isOpen, onClose, onJugadorCreated ,club_jugador_id}) => {
    console.log("club_jugador_id",club_jugador_id);
  ...
  useEffect(() => {
    const fetchClubes = async () => {
      try {
        ...
        console.log("Clubes recibidos:", response.data);
        const club = response.data.find((c) => c.id === Number(club_jugador_id));
  ...
  return (
    ...
    >
      ...
      <h2 className="modal-title">{nombreClub && `${nombreClub}`}</h2>
